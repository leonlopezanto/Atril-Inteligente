...
var optdef = {  // default values
    'jump':0, 'no_menu':0, 'repufld':0, 'noplyr':0, 'nocsr':0, 'media_height':'', 'btns':1, 'ipadr':'',
    'mstr':0, 'autscl':0, 'ctrmed':0, 'ctrnot':0, 'lncsr':0, 'opacity':0.2, 'synbox':0, 'speed':1.0,
...
function readAbcOrXML (abctxt, iniCompas) {
    //alert('ReadAbc')
    ...
    if (xs.indexOf ('//# This page') >= 0 || xs.indexOf ('play_list') >= 0) {
        alert('Estamos donde no queremos');
    ...
    if (xs.indexOf ('X:') >= 0)      { dolayout (abctxt); alert('Do Layout X:'); return }
...
function readRemoteFile(urlXML, urlAudio, iniCompas) {
  //  alert('estamos en remote');

    var freader = new FileReader();
    freader.onload = function (e) { readAbcOrXML (freader.result, iniCompas); }

    
    fetch(urlXML).then(function(r) {
         r.blob().then(function(result){
                     freader.readAsText(result);
             });       
    });

    fetch(urlAudio).then(function(r) {
        r.blob().then(function(result){

            var url = window.URL.createObjectURL (result);
            setPlayer ("hb.mp3", url);       
            });       
   });
...
function dolayout (abctxt, iniCompas) {
    //alert('DoLayout Y');
    var muziek = '', errtxt = '', abc2svg, bxs = {}, bys = {}, bars = [], xleft, times = iniCompas, nxs = [], mtxts = [];
    ...
    score.empty ();
    score.append ('<div style="height:'+ 0 +'px">&nbsp;</div>');
    ...
    //alert('Creando Wijzer con' + iniCompas);
    //msc_wz = new Wijzer (wz_xs, wz_ymin, wz_ymax, iniCompas, tixlb, lbtix, tixbts);
    msc_wz = new Wijzer (wz_xs, wz_ymin, wz_ymax, iniCompas, tixlb, lbtix, tixbts);
